...
2. **Фильтрация** (`is_vacancy`): антиспам (~70 regex-паттернов) → если совпало = НЕ вакансия
...
~70 regex-паттернов, проверяются через `re.search()`. Если **хотя бы один** совпал — сообщение НЕ вакансия.
...
| «Без опыта» | `без опыта`, `опыт не нужен`, `обучу с нуля` |
